...
if (!dojo._hasResource["dojox.grid._FocusManager"]) {
    ...
    dojo.declare("dojox.grid._FocusManager", null, {
        ...
        _scrollInfo: function (_f, _10) {
            if (_f) {
                var cl = _f, sbn = cl.view.scrollboxNode,
                    _11 = {w: sbn.clientWidth, l: sbn.scrollLeft, t: sbn.scrollTop, h: sbn.clientHeight},
        ...
        move: function (_27, _28) {
            ...
            if (this.isNavHeader()) {
                ...
                if (this.cell) {
                    var sc = this.grid.scroller, r = this.rowIndex, rc = this.grid.rowCount - 1,
                        row = Math.min(rc, Math.max(0, r + _27));
                    if (_27) {
                        if (_27 > 0) {
                    ...
                    var cc = this.grid.layout.cellCount - 1, i = this.cell.index,
                        col = Math.min(cc, Math.max(0, i + _28));
