...
if (!dojo._hasResource["dojox.charting.plot2d.Candlesticks"]) {
    ...
    (function () {
        var df = dojox.lang.functional, du = dojox.lang.utils, dc = dojox.charting.plot2d.common,
            _1 = df.lambda("item.purgeGroup()");
        dojo.declare("dojox.charting.plot2d.Candlesticks", dojox.charting.plot2d.Base, {
            defaultParams: {hAxis: "x", vAxis: "y", gap: 2, animate: null},
            optionalParams: {
                minBarSize: 1,
                maxBarSize: 1,
                stroke: {},
            ...
            render: function (_c, _d) {
                ...
                var t = this.chart.theme, f, _f, _10, ht = this._hScaler.scaler.getTransformerFromModel(this._hScaler),
                    vt = this._vScaler.scaler.getTransformerFromModel(this._vScaler),
                    _11 = Math.max(0, this._vScaler.bounds.lower), _12 = vt(_11), _13 = this.events();
                ...
                for (var i = this.series.length - 1; i >= 0; --i) {
                    ...
                    for (var j = 0; j < run.data.length; ++j) {
                        ...
                        if (v !== null) {
                            ...
                            var x = ht(v.x || (j + 0.5)) + _d.l + _f, y = _c.height - _d.b, _17 = vt(v.open),
                                _18 = vt(v.close), _19 = vt(v.high), low = vt(v.low);
                            if ("mid" in v) {
                                var mid = vt(v.mid);
                            }
                            if (low > _19) {
                            ...
                            if (_10 >= 1) {
                                var _1a = _17 > _18;
                                var _1b = {x1: _10 / 2, x2: _10 / 2, y1: y - _19, y2: y - low}, _1c = {
                                    x: 0,
                                    y: y - Math.max(_17, _18),
                                    width: _10,
                                    height: Math.max(_1a ? _17 - _18 : _18 - _17, 1)
                                };
                                shape = s.createGroup();
                                ...
                                if ("mid" in v) {
                                    _1d.createLine({
                                        x1: (_16.series.stroke.width || 1),
                                        x2: _10 - (_16.series.stroke.width || 1),
                                        y1: y - mid,
                                        y2: y - mid
                                    }).setStroke(_1a ? "white" : _16.series.stroke);
                                }
                                ...
                                if (_13) {
                                    var o = {
                                        ...
                                        y: y - Math.max(_17, _18),
                                        cx: _10 / 2,
                                        cy: (y - Math.max(_17, _18)) + (Math.max(_1a ? _17 - _18 : _18 - _17, 1) / 2),
                                        width: _10,
                                        height: Math.max(_1a ? _17 - _18 : _18 - _17, 1),
                                        data: v
                                    };
            ...
            _animateCandlesticks: function (_1e, _1f, _20) {
                dojox.gfx.fx.animateTransform(dojo.delegate({
                    shape: _1e,
                    duration: 1200,
                    transform: [{name: "translate", start: [0, _1f - (_1f / _20)], end: [0, 0]}, {
                        name: "scale",
                        start: [1, 1 / _20],
                        end: [1, 1]
                    }, {name: "original"}]
