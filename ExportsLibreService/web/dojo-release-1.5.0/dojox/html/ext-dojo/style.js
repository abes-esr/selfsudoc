...
if (!dojo._hasResource["dojox.html.ext-dojo.style"]) {
    ...
    dojo.mixin(dojox.html["ext-dojo"].style, {
        supportsTransform: true, _toPx: function (_1) {
            ...
            var to = dojo.trim(_11).replace(" top", " 0").replace("left ", "0 ").replace(" center", "50%").replace("center ", "50% ").replace(" bottom", " 100%").replace("right ", "100% ").replace(/\s+/, " "),
                _12 = to.split(" "), n = dojo.byId(_10), t = this.getTransform(n), _13 = true;
            ...
            var t = _16.replace(/\s/g, ""), n = dojo.byId(_15), _17 = t.split(")"), _18 = 1, _19 = 1,
                _1a = "DXImageTransform.Microsoft.Matrix", _1b = dojo.hasAttr, _1c = dojo.attr, PI = Math.PI,
                cos = Math.cos, sin = Math.sin, tan = Math.tan, max = Math.max, min = Math.min, abs = Math.abs,
                _1d = PI / 180, _1e = PI / 200, ct = "", _1f = "", _20 = [], x0 = 0, y0 = 0, dx = 0, dy = 0, xc = 0,
                yc = 0, a = 0, m11 = 1, m12 = 0, m21 = 0, m22 = 1, tx = 0, ty = 0, _21 = [m11, m12, m21, m22, tx, ty],
                _22 = false, ds = dojo.style, _23 = ds(n, "position") == "absolute" ? "absolute" : "relative",
                w = ds(n, "width") + ds(n, "paddingLeft") + ds(n, "paddingRight"),
                h = ds(n, "height") + ds(n, "paddingTop") + ds(n, "paddingBottom"), _24 = this._toPx;
            ...
            if (dojo.isIE < 8) {
                n.style.zoom = "1";
                if (_23 != "absolute") {
                    var _29 = ds(_15.parentNode, "width"), tw = abs(w * m11), th = abs(h * m12),
                        _2a = max(tw + th, max(max(th, tw), 0));
