...
if (!dojo._hasResource["dojox.widget.UpgradeBar"]) {
    ...
    dojo.declare("dojox.widget.UpgradeBar", [dijit._Widget, dijit._Templated], {
                ...
                dojo.forEach(_2.childNodes, function (n) {
                    if (n.nodeType == 1) {
                        ...
                        this.notifications.push({
                            message: n.innerHTML, validate: function () {
                                var _4 = true;
                                try {
                                    _4 = dojo.eval(_3);
                                } catch (e) {
        ...
        show: function () {
            ...
            if (!this._showAnim) {
                this._showAnim = dojo.fx.combine([dojo.animateProperty({
                    node: dojo.body(),
                    duration: 500,
                    properties: {marginTop: this._bodyMarginTop + this._size}
                }), dojo.animateProperty({
                    node: this.domNode,
                    duration: 500,
                    properties: {height: this._size, opacity: 1}
                })]);
            }
            this._showAnim.play();
        },
        hide: function () {
            if (!this._hideAnim) {
                this._hideAnim = dojo.fx.combine([dojo.animateProperty({
                    node: dojo.body(),
                    duration: 500,
                    properties: {marginTop: this._bodyMarginTop}
