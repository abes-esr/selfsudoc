...
if (!dojo._hasResource["dojox.drawing.annotations.BoxShadow"]) {
    ...
    dojox.drawing.annotations.BoxShadow = dojox.drawing.util.oo.declare(function (_1) {
        ...
        showing: true, render: function () {
            ...
            var o = this.options, _3 = o.size, _4 = o.mult,
                d = this.method == "createForPath" ? this.stencil.points : this.stencil.data, r = d.r || 1, p = o.place,
                c = o.color;
            ...
            var sh = _5 * _6 / 4, _8 = /B/.test(p) ? sh : /T/.test(p) ? sh * -1 : 0,
                _9 = /R/.test(p) ? sh : /L/.test(p) ? sh * -1 : 0;
            var _a = true;
            for (var i = 1; i <= _5; i++) {
                var _b = i * _6;
            ...
            var sh = _f * _10 / 4, shy = /B/.test(p) ? sh : /T/.test(p) ? sh * -1 : 0,
                shx = /R/.test(p) ? sh : /L/.test(p) ? sh * -1 : 0;
            for (var i = 1; i <= _f; i++) {
                var _11 = i * _10;
                this.container.createLine({
                    x1: d.x1 + shx,
                    y1: d.y1 + shy,
                    x2: d.x2 + shx,
                    y2: d.y2 + shy
                }).setStroke({width: _11, color: c, cap: "round"});
            }
        }, createForEllipse: function (o, _12, _13, d, r, p, c) {
            var sh = _12 * _13 / 8, shy = /B/.test(p) ? sh : /T/.test(p) ? sh * -1 : 0,
                shx = /R/.test(p) ? sh * 0.8 : /L/.test(p) ? sh * -0.8 : 0;
            for (var i = 1; i <= _12; i++) {
                var _14 = i * _13;
                this.container.createEllipse({
                    cx: d.cx + shx,
                    cy: d.cy + shy,
                    rx: d.rx - sh,
                    ry: d.ry - sh,
                    r: r
                }).setStroke({width: _14, color: c});
            }
        }, createForRect: function (o, _15, _16, d, r, p, c) {
            var sh = _15 * _16 / 2, shy = /B/.test(p) ? sh : /T/.test(p) ? 0 : sh / 2,
                shx = /R/.test(p) ? sh : /L/.test(p) ? 0 : sh / 2;
            for (var i = 1; i <= _15; i++) {
                var _17 = i * _16;
                this.container.createRect({
                    x: d.x + shx,
                    y: d.y + shy,
                    width: d.width - sh,
                    height: d.height - sh,
                    r: r
                }).setStroke({width: _17, color: c});
            }
            ...
            var sh = _1b * _1c / 4, shy = /B/.test(p) ? sh : /T/.test(p) ? sh * -1 : 0,
                shx = /R/.test(p) ? sh : /L/.test(p) ? sh * -1 : 0;
            var _1d = true;
            for (var i = 1; i <= _1b; i++) {
                var _1e = i * _1c;
                ...
                if (dojox.gfx.renderer == "svg") {
                    ...
                    this.container.createPath(_1f.join(", ")).setStroke({
                        width: _1e,
                        color: c,
                        cap: "round"
                    }).setFill(c);
                ...
                this.container.createLine({x1: sp[0].x, y1: sp[0].y, x2: pts[0].x, y2: pts[0].y}).setStroke({
                    width: _1e,
                    color: c,
                    cap: "round"
                });
