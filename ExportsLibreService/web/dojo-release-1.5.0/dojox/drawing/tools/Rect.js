...
if (!dojo._hasResource["dojox.drawing.tools.Rect"]) {
    ...
    dojox.drawing.tools.Rect = dojox.drawing.util.oo.declare(dojox.drawing.stencil.Rect, function () {
        ...
        draws: true, onDrag: function (_1) {
            var s = _1.start, e = _1;
            var x = s.x < e.x ? s.x : e.x, y = s.y < e.y ? s.y : e.y, w = s.x < e.x ? e.x - s.x : s.x - e.x,
            ...
            if (!this.shape) {
                ...
                this.setPoints([{x: s.x, y: s.y}, {x: s.x + e, y: s.y}, {x: s.x + e, y: s.y + e}, {
                    x: s.x,
                    y: s.y + e
                }]);
                this.render();
    ...
    dojox.drawing.tools.Rect.setup = {
        ...
        tooltip: "<span class=\"drawingTipTitle\">Rectangle Tool</span><br/>" + "<span class=\"drawingTipDesc\">SHIFT - constrain to square</span>",
        iconClass: "iconRect"
    };
